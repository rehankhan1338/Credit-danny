...
function emitNode(node, sourceFile, depth, inSvg, ctx, parentTag) {
  ...
  if (tag === "script") {
    ...
    if (content.length < 200 && /documentElement\.(classList\.add|className)/.test(content)) {
      // pre-paint JS-detection class — must run before first paint
      return `${pad}<script ${attrs}${attrs ? " " : ""}dangerouslySetInnerHTML={{ __html: ${JSON.stringify(content)} }} />`;
    }
    ctx.usesNextScript = true;
